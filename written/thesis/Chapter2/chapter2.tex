...
En la figura \ref{fig:requirements} se muestran todos los requerimientos de alto, medio y bajo nivel. Puede decirse que los L0 son requerimientos padres de los L1 y estos a su vez de los L2. A su vez, los de bajo nivel están subdivididos en requerimientos de RF, baja frecuencia y masa. En esta primera versión no se han tenido en cuenta requerimientos térmicos ni mecánicos por tratarse de un modelo de desarrollo.
...
\begin{figure}[H]
  ...
  \begin{tabular}[t]{|@{}p{4.6cm}|}
    ...
    \begin{enumerate}[label=L1.\arabic*, nosep]
      ...
      \req{Medición de Ganancia} \label{req:l1_gain}
      \req{Medición de Fase} \label{req:l1_phase}
      \req{Independencia con distancia} \label{req:l1_independence}
    \end{enumerate} \tabularnewline
  ...
  \begin{tabular}[t]{|p{4.6cm}|}
    ...
    \centering \textit{Subsistema RF} \tabularnewline

    \hline
    \begin{enumerate}[label=L2.\arabic*, nosep, series=qa]
      \req{Frecuencia central} \label{req:l2_f0}
      \req{Ancho de banda} \label{req:l2_bw}
      \req{Potencia de transmisión RF} \label{req:l2_txPower}
      \req{Tipo Antena} \label{req:l2_antenna}
      \req{Número de Antenas} \label{req:l2_antQuantity}
      \req{Polarización} \label{req:l2_polarization}
    ...
    \centering \textit{Baja Frecuencia} \tabularnewline
    ...
    \begin{enumerate}[resume*=qa]
      ...
      \req{Tensión de trabajo} \label{req:l2_voltage}
    \end{enumerate} \tabularnewline
    \hline

    \centering \textit{Masa} \tabularnewline
    ...
    \centering \textit{Software} \tabularnewline

    \hline
    \begin{enumerate}[resume*=qa]
      \req{Procesamiento} \label{req:l2_realTime}
      \req{Visualización} \label{req:l2_measurements}
      \req{Gráficos} \label{req:l2_plots}
      \req{Funcionalidades} \label{req:l2_buttons}
    \end{enumerate} \tabularnewline
    \hline
...
A continuación se detallan los requerimientos ilustrados en la figura \ref{fig:requirements}. Los cuales son definidos a lo largo del desarrollo del presente trabajo de tesis.
...
\begin{figure}[H]
 \centering
 \includegraphics[width=10cm]{nPortsNetwork}
...
Invirtiendo las ecuaciones de \ref{eq:voltages}, se obtienen las amplitudes de las ondas de tensión en función de las tensiones y corrientes totales del puerto.
...
Un elemento específico de la matriz de scattering puede ser determinado como
...
\begin{figure}
 \centering
 \includegraphics[width=6cm]{powerWaves}
...
Invirtiendo las variables de la ecuación \ref{eq:powerWaves} se obtienen las tensiones y corrientes totales en función de las amplitudes de las ondas de potencia.
